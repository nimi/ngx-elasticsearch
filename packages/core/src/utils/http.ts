...
function rejectOnError(response: Response) {
  if (!response.ok) {
    throw response;
  }
...
export class HttpClient {
   ...
   */
  defaults: RequestInit | null = null;
   ...
   * @chainable
   */
  configure(config: RequestInit | HttpClientConfiguration | Function): HttpClient {
  ...
  fetch(input: Request|string, init?: RequestInit): Promise<Response> {
    ...
    let promise = processRequest(request, this.interceptors)
      .then((result: any) => {
        ...
        return request.then((_result: any) => processResponse(response, this.interceptors, _result));
      });

    return trackRequestEndWith.call(this, promise);
  }

  get(url: Request | string, options?: any): Promise<Response> {
  ...
  delete(url: Request | string, options?: any): Promise<Response> {
    return this.fetch(url, { ...options,  method: 'delete' });
  }

  options(url: Request | string, options?: any): Promise<Response> {
    return this.fetch(url, { ...options,  method: 'options' });
  }

  head(url: Request | string, options?: any): Promise<Response> {
...
function trackRequestEndWith(promise: Promise<any>) {
  let handle = trackRequestEnd.bind(this);
  promise.then(handle, handle);
  return promise;
}

function parseHeaderValues(headers: any) {
  let parsedHeaders = {};
...
function buildRequest(input: any, init: any) {
...
function setDefaultHeaders(headers: any, defaultHeaders: any) {
...
function processRequest(request: any, interceptors: any) {
  return applyInterceptors(request, interceptors, 'request', 'requestError');
}

function processResponse(response: any, interceptors: any[], request: any) {
  return applyInterceptors(response, interceptors, 'response', 'responseError', request);
}

function applyInterceptors(
  input: any, interceptors: any[], successName: string, errorName: string, ...interceptorArgs: any[]
) {
  return (interceptors || [])
