...
var FacetAccessor = (function (_super) {
    ...
    function FacetAccessor(key, options) {
        ...
        _this.loadAggregations = lodash_1.isUndefined(_this.options.loadAggregations) ? true : _this.options.loadAggregations;
    ...
    FacetAccessor.prototype.getBuckets = function () {
        ...
        var keyIndex = lodash_1.keyBy(rawBuckets, 'key');
        var inIndex = function (filter) { return !!keyIndex[filter]; };
        var missingFilters = [];
        lodash_1.each(this.state.getValue(), function (filterKey) {
    ...
    FacetAccessor.prototype.buildSharedQuery = function (query) {
        ...
        var filterTerms = lodash_1.map(filters, function (filter) {
    ...
    FacetAccessor.prototype.buildOwnQuery = function (query) {
        if (!this.loadAggregations) {
            ...
            return query
                ...
                query.getFiltersWithoutKeys(excludedKey)].concat(this.fieldContext.wrapAggregations(query_1.TermsBucket(this.key, this.key, lodash_1.omitBy({
                size: this.size,
                order: this.getOrder(),
                include: this.options.include,
                exclude: this.options.exclude,
                min_doc_count: this.options.min_doc_count
            }, lodash_1.isUndefined)), query_1.CardinalityMetric(this.key + '_count', this.key)))));
        }
    };
